...
describe.each([
  ...
  const assetDeploymentId =
    envKey === 'ASSET_DEPLOYMENT_ID' ? `static-${deploymentId}` : deploymentId
    ...
    async ({ urlPath }) => {
      ...
      for (const script of scripts) {
        if (script.attribs.src) {
          expect(script.attribs.src).toContain('dpl=' + assetDeploymentId)
        }
      }
      ...
      for (const link of links) {
        if (link.attribs.href && link.attribs.rel !== 'expect') {
          if (link.attribs.as === 'font') {
            expect(link.attribs.href).not.toContain('dpl=' + assetDeploymentId)
          } else {
            expect(link.attribs.href).toContain('dpl=' + assetDeploymentId)
          }
        }
      }
      ...
      try {
        expect(
          requests.every((item) => item.includes('dpl=' + assetDeploymentId))
        ).toBe(true)
      } finally {
